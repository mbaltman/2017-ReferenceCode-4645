...
public class MakeParallel extends Command {
    ...
    protected void execute() {
    	...
    	double drivingDistance = Math.abs(angDif * RobotMap.RADIUS);
		...
		if (finalFR && finalFL && finalBR && finalBL) {
			...
			SwerveDrive.drivingMotorFrontLeft.set(SwerveDrive.drivingMotorFrontLeft.getPosition() + drivingDistance);
			SwerveDrive.drivingMotorBackRight.changeControlMode(TalonControlMode.Position);
			SwerveDrive.drivingMotorBackRight.set(SwerveDrive.drivingMotorFrontLeft.getPosition() + drivingDistance);
